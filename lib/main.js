...
function scrollToVarDecl( textEditor, varname, range ) {
  ...
  while ( n >= 0 ) {
    ...
    if ( m = line.match( new RegExp( "\\" + varname + "\s*[^=]?=" )) )
      break;
  }
  ...
  textEditor.setCursorBufferPosition([n+1, line.indexOf( varname )]);
  ...
  getSuggestionForWord(textEditor: TextEditor, text: string, range: Range): HyperclickSuggestion {
    if ( isPHP( textEditor )) {
      ...
      if ( p ) {
        ...
        // p = {
        //       underlineRange: the range that we **should** return for underlining the whole file name
        //       filename: the filename that was matched
        //     }

        return {
          // The range(s) to underline as a visual cue for clicking.
          // TODO figure out how to underline the whole filename
          range,
          // The function to call when the underlined text is clicked.
          callback() {
            openFile( textEditor, p.filename );
          }
        }
      }
      ...
      if ( t ) {
        return {
          ...
          // The function to call when the underlined text is clicked.
          callback() {
            scrollToVarDecl( textEditor, t.varname, range );
          }
        }
      }
    }
